...
addFilter(
  ...
  (settings, name) => name === 'core/heading' ? addExcludeFromTocAttribute(settings) : settings
);
...
const withInspectorControls = createHigherOrderComponent((BlockEdit) => (props) => {
  ...
  return (
    <>
      <BlockEdit {...props} />
      <ExcludeFromTocToggle attributes={props.attributes} setAttributes={props.setAttributes} />
    </>
  );
