...
class TextStyleTest extends WP_UnitTestCase {
	...
	public function test_vk_blocks_get_color_value() {
		$test_data = array(
			array(
				'value'  => '#F0F0F1',
				'correct' => '#F0F0F1',
		...
		print PHP_EOL;
		print '------------------------------------' . PHP_EOL;
		print 'vk_blocks_get_color_value()' . PHP_EOL;
		print '------------------------------------' . PHP_EOL;
		foreach ( $test_data as $test_value ) {

			$return  = vk_blocks_get_color_value( $test_value['value'] );
	...
	public function test_vk_blocks_get_hex_to_rgb() {
		$test_data = array(
			array(
				'value'  => '#F0F0F1',
		...
		foreach ( $test_data as $test_value ) {
			...
			$return  = vk_blocks_get_hex_to_rgb( $test_value['value'], '0.7' );
			$correct = $test_value['correct'];
