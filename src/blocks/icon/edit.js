	...
	if (!parent.length) {
		commonInspectorControls = (
			<>
				...
				<p className={`mt-0 mb-2`}>{__('Margin', 'vk-blocks-pro')}</p>
				<div className="vk_icon_custombox">
					<TextControl
						className={`vk_icon_custombox_number`}
						value={iconMargin}
				...
				<p className="mt-0 mb-2">{__('Style', 'vk-blocks-pro')}</p>
				<ToggleGroupControl
					value={iconType}
					onChange={(value) => {
				...
				>
					...
					<ToggleGroupControlOption
						value="1"
						label={__('Icon & Frame', 'vk-blocks-pro')}
					/>
					<ToggleGroupControlOption
						value="2"
						label={__('Icon only', 'vk-blocks-pro')}
					/>
				</ToggleGroupControl>
			</>
		);
