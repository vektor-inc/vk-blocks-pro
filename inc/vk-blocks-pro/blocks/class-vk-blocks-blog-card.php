...
class VK_Blocks_Blog_Card {
	...
	/**
	 * Get Blog Card HTML
	 * Blog Card 2が不要ならこの関数は要らない
	 *
	 * @param url     $url URL.
	 * @param boolean $clear_cache キャッシュクリアするか否か.
	 * @param array   $attributes attributes.
	 * @return string vk_get_blog_card_html
	 */
	public static function vk_get_blog_card_html( $url, $clear_cache = false, $attributes ) {
		$blog_card_data = static::vk_get_blog_card_data( $url, $clear_cache );
		...
		$description = $blog_card_data['excerpt'];
		$thumbnail   = $blog_card_data['featured_image'];
		$favicon     = $blog_card_data['site_logo'];
		$site_name   = $blog_card_data['site_title'];
		$domain      = $blog_card_data['domain'];

		ob_start();
		?>
		<?php if ( $thumbnail && ! empty( $attributes['cardType'] ) && 'left-text-right-image' === $attributes['cardType'] ) : ?>
		...
		<?php endif; ?>
		<?php if ( $thumbnail && ! empty( $attributes['cardType'] ) && 'large-image-and-image-lower-character' === $attributes['cardType'] ) : ?>
			<div class="blog-card-image-outer">
				<a href="<?php echo esc_url( $url ); ?>" class="blog-card-image-frame">
					<img class="blog-card-image-src" src="<?php echo esc_url( $thumbnail ); ?>" alt="">
				</a>
		...
		<?php if ( $thumbnail && empty( $attributes['cardType'] ) ) : ?>
			<div class="blog-card-image-outer">
				<a href="<?php echo esc_url( $url ); ?>" class="blog-card-image-frame">
					<img class="blog-card-image-src" src="<?php echo esc_url( $thumbnail ); ?>" alt="">
				</a>
