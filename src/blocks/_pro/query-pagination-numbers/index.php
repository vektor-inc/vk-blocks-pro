 ...
 */
function vk_blocks_render_block_query_pagination_numbers( $attributes, $content, $block ) {
	...
	if ( isset( $block->context['query']['inherit'] ) && $block->context['query']['inherit'] ) {
		...
		$block_query = new WP_Query( vk_blocks_build_query_vars_from_query_block( $block, $page ) );
		// `paginate_links` works with the global $wp_query, so we have to
		// temporarily switch it with our custom query.
		// paginate_links` はグローバルな $wp_query で動作するため、以下のようにします。一時的にカスタムクエリに切り替えます。
 ...
 */
function vk_blocks_register_block_query_pagination_numbers() {
	register_block_type(
		__DIR__,
		array(
			'editor_script'   => 'vk-blocks-build-js',
			'render_callback' => 'vk_blocks_render_block_query_pagination_numbers',
		)
	);
}
add_action( 'init', 'vk_blocks_register_block_query_pagination_numbers', 99 );
