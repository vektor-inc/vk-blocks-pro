 ...
 */
function vk_blocks_query_pagination_next_render_callback( $attributes, $content, $block ) {
	$page_key = isset( $block->context['queryId'] ) ? 'query-' . $block->context['queryId'] . '-page' : 'query-page';
	$page     = empty( $_GET[ $page_key ] ) ? 1 : (int) $_GET[ $page_key ];
	$max_page = isset( $block->context['query']['pages'] ) ? (int) $block->context['query']['pages'] : 0;

	$wrapper_attributes = get_block_wrapper_attributes();
	$default_label      = __( 'Next Page', 'vk-blocks' );
	...
	if ( isset( $block->context['query']['inherit'] ) && $block->context['query']['inherit'] ) {
		...
		$custom_query           = new WP_Query( vk_blocks_build_query_vars_from_query_block( $block, $page ) );
...
/**
 * Register query-pagination-next.
 *
 * @return void
 */
function vk_blocks_register_block_query_pagination_next() {
	register_block_type(
		__DIR__,
		array(
			'editor_script'   => 'vk-blocks-build-js',
			'render_callback' => 'vk_blocks_query_pagination_next_render_callback',
		)
	);
}
add_action( 'init', 'vk_blocks_register_block_query_pagination_next', 99 );
